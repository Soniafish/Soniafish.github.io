...
def apiUsers():
    ...
    if 'userid' in session:
        # 篩選資料表的資料
        result=cursor.execute("SELECT * FROM user where username='"+username+"'")
        # print(result)
        if result:   # 會員帳號存在
            select_data=cursor.fetchone()   #取得使用者名字
            return json.dumps({
...
def apiUser():
    ...
    newName=insertValues["name"]

    # 更新資料表內容
    result=cursor.execute("UPDATE user SET name='"+newName+"' where username='"+session['userid']+"'")
    cnnt.commit()
    # print(result)
